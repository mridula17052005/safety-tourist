...
import {
  Building2, Navigation, Phone, Star, Search, Loader2, ChevronLeft,
  MapPin, Clock,
} from 'lucide-react';
...
export function HospitalsPage() {
  ...
  const { coords, places, loading, searching, error, mapRef, searchNearby } = useNearbySearch('hospital', 'hospital');

  const mapMarkers = places.map((p) => ({ lat: p.lat, lng: p.lng, title: p.name, label: 'H' }));
    ...
    <div className="max-w-7xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">Closest hospitals and medical centers to your location</p>
        </div>
      </div>

      <div className="flex items-center justify-between">
        <Badge className="bg-red-100 text-red-700">{places.length} hospitals found</Badge>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
          {loading ? (
            ...
            <Card className="p-6 text-center text-slate-400 text-sm">No hospitals found nearby. Try refreshing.</Card>
          ) : (
            places.map((place) => (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-3 mt-3">
                      ...
                      <a href="tel:911" className="text-xs font-medium text-red-600 hover:text-red-700 flex items-center gap-1">
